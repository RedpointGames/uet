namespace Redpoint.Collections
{
    ...
    public static class ClassifyingLinqExtensions
    {
        ...
        private class ClassifyingEnumerable<TIn, TOut> : IClassifiableAsyncEnumerable<TIn, TOut>
        {
            ...
            private async IAsyncEnumerable<TOut> WrapMapper(IAsyncEnumerable<TIn> inputs, Func<TIn, TOut> handler)
            {
            ...
            private async IAsyncEnumerable<TOut> WrapMapper(IAsyncEnumerable<TIn> inputs, Func<TIn, Task<TOut>> handler)
            {
                await foreach (var input in inputs)
                {
                    yield return await handler(input);
                }
            }

            public IClassifiableAsyncEnumerable<TIn, TOut> AndForClassification(string classification, Func<TIn, TOut> handler)
            {
                _connections[classification] = inputs => WrapMapper(inputs, handler);
                return this;
            }
            ...
            private async Task<(IAsyncEnumerator<T> enumerator, bool moved, T? current)> MoveNextAndGetCurrentAsync<T>(IAsyncEnumerator<T> enumerator)
            {
            ...
            {
                var inputEnumerator = new ClassifyingInputEnumerator<TIn>(_source.GetAsyncEnumerator(), _classifier);
                ...
                }).ToDictionary(k => k, v => MoveNextAndGetCurrentAsync(v));
                while (enumerators.Count > 0)
                {
                    var nextValue = await await Task.WhenAny(enumerators.Values);
                    if (!nextValue.moved)
                    {
                    ...
                    else
                    {
                        ...
                        enumerators[nextValue.enumerator] = MoveNextAndGetCurrentAsync(nextValue.enumerator);
                    }
                }
            }
            ...
            private class ClassifyingInputEnumerator<T>
            {
                private readonly IAsyncEnumerator<T> _source;
                private readonly Func<T, Task<string>> _classifier;
                private SemaphoreSlim _iterationMutex;
                ...
                {
                    _iterationMutex = new SemaphoreSlim(1);
                ...
                {
                    await _iterationMutex.WaitAsync();
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            var nextClassification = await _classifier(_source.Current);
            ...
            private class ClassifyingOutputEnumerator<T> : IAsyncEnumerator<T>
            {
                ...
                {
                    var (moved, current) = await _enumerator.GetNextAsync(_classification, _cancellationToken);
                    Current = current;
                    return moved;
                }
            }

            private class ClassifyingOutputEnumerable<T> : IAsyncEnumerable<T>
            {
                private readonly string _classification;
                private readonly ClassifyingInputEnumerator<T> _enumerator;
