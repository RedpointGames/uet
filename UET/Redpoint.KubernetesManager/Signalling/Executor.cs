namespace Redpoint.KubernetesManager.Signalling
{
    ...
    internal class Executor
    {
        ...
        private class WrappingRegistrationContext : IRegistrationContext
        {
            ...
            {
                if (!_executor._signalDispatch.ContainsKey(signalType))
                {
                    _executor._signalDispatch.Add(signalType, new List<(Type, SignalDelegate)>());
                }
                _executor._signalDispatch[signalType].Add((_componentType, callback));
            }
        }
