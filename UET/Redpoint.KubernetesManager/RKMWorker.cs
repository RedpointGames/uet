        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            if (!File.Exists(roleFile))
            {
                if (_commandLine.Arguments.Contains("--controller"))
                {
                    ...
                    await File.WriteAllTextAsync(roleFile, $"controller");
                }
                else if (_commandLine.Arguments.Contains("--node"))
                {
                    ...
                    await File.WriteAllTextAsync(roleFile, $"node,{address}");
                }
                else
                {
                    ...
                    if (existingControllerAddress != null)
                    {
                        ...
                        await File.WriteAllTextAsync(roleFile, $"node,{existingControllerAddress}");
                    }
                    else
                    {
            ...
            var role = await File.ReadAllTextAsync(roleFile);
            RoleType roleType;
