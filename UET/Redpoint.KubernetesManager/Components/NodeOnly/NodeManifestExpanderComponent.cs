namespace Redpoint.KubernetesManager.Components.NodeOnly
{
    ...
    internal class NodeManifestExpanderComponent : IComponent
    {
        ...
        private async Task OnStartedAsync(IContext context, IAssociatedData? data, CancellationToken cancellationToken)
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "certs", "ca", $"ca.pem"), nodeManifest.CertificateAuthority);
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "certs", "nodes", $"node-{nodeManifest.NodeName}.pem"), nodeManifest.NodeCertificate);
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "certs", "nodes", $"node-{nodeManifest.NodeName}.key"), nodeManifest.NodeCertificateKey);
            ...
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "kubeconfigs", "nodes", $"node-{nodeManifest.NodeName}.kubeconfig"), nodeManifest.NodeKubeletConfig.Replace("__CONTROLLER_ADDRESS__", controllerAddress.ToString()));
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "kubeconfigs", "components", $"component-kube-proxy.kubeconfig"), nodeManifest.KubeProxyConfig.Replace("__CONTROLLER_ADDRESS__", controllerAddress.ToString()));
            await File.WriteAllTextAsync(Path.Combine(_pathProvider.RKMRoot, "kubeconfigs", "components", $"component-calico-windows-node.kubeconfig"), nodeManifest.CalicoWindowsConfig.Replace("__CONTROLLER_ADDRESS__", controllerAddress.ToString()));
