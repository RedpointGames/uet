namespace Redpoint.KubernetesManager.Components.ControllerOnly
{
    ...
    internal class KubernetesResourceProvisioningComponent : IComponent
    {
        ...
        private async Task OnStartedAsync(IContext context, IAssociatedData? data, CancellationToken cancellationToken)
        {
            ...
            try
            {
                await kubernetes.RbacAuthorizationV1.ReadClusterRoleAsync("system:kube-apiserver-to-kubelet");
            }
            catch (HttpOperationException ex) when (ex.Response.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                await kubernetes.RbacAuthorizationV1.CreateClusterRoleAsync(new k8s.Models.V1ClusterRole
                {
            ...
            try
            {
                await kubernetes.RbacAuthorizationV1.ReadClusterRoleBindingAsync("system:kube-apiserver");
            }
            catch (HttpOperationException ex) when (ex.Response.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                await kubernetes.RbacAuthorizationV1.CreateClusterRoleBindingAsync(new k8s.Models.V1ClusterRoleBinding
                {
                    Metadata = new k8s.Models.V1ObjectMeta
                    {
                    ...
                    Subjects = new List<k8s.Models.V1Subject>
                    {
                        new k8s.Models.V1Subject
                        {
                            ApiGroup = "rbac.authorization.k8s.io",
                            Kind = "User",
                            Name = "kubernetes"
