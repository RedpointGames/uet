        ...
        private async Task OnStartedAsync(IContext context, IAssociatedData? data, CancellationToken cancellationToken)
        {
            ...
            else if (OperatingSystem.IsWindows())
            {
                await _resourceManager.ExtractResource(
                    ...
                    new Dictionary<string, string>
                    {
                        { "__CA_CERT_FILE__", _certificateManager.GetCertificatePemPath("ca", "ca").Replace("\\", "\\\\") },
                        { "__NODE_CERT_FILE__", _certificateManager.GetCertificatePemPath("nodes", $"node-{nodeName}").Replace("\\", "\\\\") },
                        { "__NODE_KEY_FILE__", _certificateManager.GetCertificateKeyPath("nodes", $"node-{nodeName}").Replace("\\", "\\\\") },
