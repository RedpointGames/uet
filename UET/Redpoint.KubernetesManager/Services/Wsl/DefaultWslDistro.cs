namespace Redpoint.KubernetesManager.Services.Windows
{
    ...
    internal class DefaultWslDistro : IWslDistro
    {
        ...
        private async Task<bool> EnsureInstalled(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    var x64File = Directory.GetFiles(Path.Combine(_pathProvider.RKMRoot, "ubuntu-package")).FirstOrDefault(x => x.EndsWith("_x64.appx"));
                    if (x64File == null)
                    {
