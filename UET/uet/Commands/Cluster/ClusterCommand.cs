    ...
    internal sealed class ClusterCommand
    {
        public static Command CreateClusterCommand()
        {
            ...
            command.FullDescription =
                ...
                Kubernetes clusters set up by this command are intended for build and automation workloads on trusted networks, with trusted code. They are not suitable for Internet-facing workloads, and actively trade security and isolation for ease-of-use and compatibility with the greatest range of platforms.

                If you have no cluster set up on your network already, you must use 'uet cluster create' on a Linux machine; this machine will run the Kubernetes controller components that can only run on Linux. It will also join the cluster as a normal Kubernetes node capable of running workloads.

                After the cluster has been created, you can join Windows and Linux (and in the future, macOS) machines to the cluster using 'uet cluster join'.
                ...
                """;
            command.AddCommand(ClusterCreateCommand.CreateClusterCreateCommand());
            command.AddCommand(ClusterJoinCommand.CreateClusterJoinCommand());
