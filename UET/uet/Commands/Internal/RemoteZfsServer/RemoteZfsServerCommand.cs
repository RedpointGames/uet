    ...
    internal sealed class RemoteZfsServerCommand
    {
        ...
        private sealed class RemoteZfsServerCommandInstance : RemoteZfs.RemoteZfsBase, ICommandInstance
        {
                ...
                ServerCallContext context)
            {
                ...
                catch (Exception ex) when (context.CancellationToken.IsCancellationRequested && !(ex is OperationCanceledException))
                {
                    _logger.LogError(ex, $"Exception while connection is already closed: {ex.Message}");
                }
            }
        }
    }
