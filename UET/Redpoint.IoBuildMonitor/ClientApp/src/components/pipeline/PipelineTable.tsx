...
function PipelineDescriptionRenderer(props: { pipeline: Pipeline }) {
  let countdown = null;
  if (
    props.pipeline.startedUtcMillis !== null &&
    props.pipeline.estimatedUtcMillis !== null
  ) {
    countdown = (
      <>
        <br />
...
export default function PipelineTable(props: {
  pipelines: Pipeline[];
  showFinished?: true;
}) {
  ...
  columns.push({
    ...
    render: (item: Pipeline) => (
      <span style={{ whiteSpace: "nowrap" }}>
        <PipelineDescriptionRenderer pipeline={item} />
      </span>
    ),
  });
  if (props.showFinished) {
    columns.push({
      ...
      render: (item: Pipeline) => (
        <span style={{ whiteSpace: "nowrap" }}>
          <PipelineFinishedRenderer pipeline={item} />
        </span>
      ),
    });
  }
