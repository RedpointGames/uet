import { useEuiTheme, withEuiTheme, type UseEuiTheme } from "@elastic/eui";
...
export function ProgressGlob(
  props:
    | { type: "indeterminate" | "success" | "failed" | "created" | "stopped" }
    | { type: "inprogress"; start: number; estimate: number },
) {
  ...
  public render() {
    ...
    if (endMillis > 0) {
      ...
      if (progressAmount >= 100) {
        progressAmount = 100;
        return <EuiProgress css={progressBarStyle} color="primary" />;
      }
      return (
        <EuiProgress
          css={progressBarStyle}
...
export const ProgressNibblin = withEuiTheme(
  class ProgressNibblinImpl extends React.Component<ProgressNibblinProps> {
    private painting: boolean = false;
    ...
    public render() {
      let spinnerCss = css`
        margin: 0.15rem;
        color: ${this.props.theme.euiTheme.colors.primary};
      `;
