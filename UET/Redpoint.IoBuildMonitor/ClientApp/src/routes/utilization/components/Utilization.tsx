...
import { DashboardStats, UtilizationStats } from "../../../data/DataTypes";
...
export const Utilization = InjectGuarded(
  {
          ...
          <EuiPanel>
            ...
            <EuiFlexGroup direction="row">
              ...
              <EuiFlexItem grow={false}>
                ...
                <EuiStat title={row.desiredCapacity} description="Desired Capacity (50% of the time)" />
                <Chart key={row.tag}>
                  <Settings
                    theme={{
                      ...EUI_CHARTS_THEME_DARK.theme,
                      chartMargins: {
                        ...EUI_CHARTS_THEME_DARK.theme.chartMargins,
                        left: 0,
                      },
                      chartPaddings: {
                        ...EUI_CHARTS_THEME_DARK.theme.chartPaddings,
                        left: -10,
                      }
                    }}
                    showLegend={false}
    ...
    if (entries.length === 0) {
          ...
          body={
            <>
              <p>
                ...
                been run in the last 24 hours, or Io is yet to process
                historical build data.
              </p>
              <p>
                This page will automatically update with utilization metrics
      ...
      callout = (
        <>
          ...
          >
            <p>
              Utilization metrics will not show builds that are created or
              pending in the current build queue. This is because GitLab does
              not assign runner tags to a build until it starts running, so we
              don't know which graph to assign those builds to until the builds
              actually start.
            </p>
            <p>
              After the builds have started running, the utilization metrics
              processor will go back and fix up the historical data so that it
              is correct.
            </p>
          </EuiCallOut>
          <EuiSpacer size="l" />
      ...
      >
        <EuiFlexGroup>
          <EuiFlexItem>
            <EuiText size="xs">
              <p>
                Utilization metrics show you how much your build agents are
                being used by builds over time. It is designed to help you
                decide when you need to invest in more build infrastructure.
              </p>
              <p>
                For this reason, utilization metrics exclude any build jobs that
                take less than 60 seconds to run. These quick build jobs are
                often constrained on job setup, rather than compute power in
                your build agent cluster.
              </p>
