...
namespace Tpm2Lib
{
            ...
            do try
                {
                    ...
                    if (repeat && resultCode == TpmRc.Success)
                    {
                        ...
                        var actualParms = (TpmStructureBase)TpmStructureRegistry.Create(inParms.GetType());
    ...
    public class CommandProcessor
    {
        ...
        public static void Fragment(TpmStructureBase s, uint numHandles, out TpmHandle[] handles, out byte[] parms)
        {
            ...
            try
            {
                fields = TpmStructureRegistry.Get(s.GetType()).GetMembers(BindingFlags.Public | BindingFlags.Instance);
            }
            catch (Exception)
            {
                throw;
