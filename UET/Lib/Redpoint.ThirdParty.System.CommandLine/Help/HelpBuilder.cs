...
namespace System.CommandLine.Help
{
    ...
    public partial class HelpBuilder
    {
        ...
        /// <param name="sizeItems">The help items used to configure column widths.</param>
        public void WriteColumns(IReadOnlyList<TwoColumnHelpRow> items, HelpContext context, IReadOnlyList<TwoColumnHelpRow>? sizeItems = null)
        {
            if (items.Count == 0)
            {
                return;
            ...
            int firstColumnWidth = sizeItems.Select(x => x.FirstColumnText.Length).Max();
            int secondColumnWidth = sizeItems.Select(x => x.SecondColumnText.Length).Max();
            ...
            if (firstColumnWidth + secondColumnWidth + Indent.Length + Indent.Length > windowWidth)
            {
                ...
                {
                    firstColumnWidth = sizeItems.SelectMany(x => WrapText(x.FirstColumnText, firstColumnMaxWidth).Select(x => x.Length)).Max();
                }
