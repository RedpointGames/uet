...
public partial class HelpBuilder
{
    ...
    public static class Default
    {
        ...
        public static HelpSectionDelegate OptionsSection() =>
            ctx =>
            {
                var sections = ctx.Command.Options.Where(x => !x.IsHidden).Select(x => x.ArgumentGroupName).Distinct();

                List<TwoColumnHelpRow> sizeOptions = new();
                List<(string? section, List<TwoColumnHelpRow>)> groupedOptions = new();
                foreach (var section in sections)
                {
                    ...
                    {
                        if (!option.IsHidden && uniqueOptions.Add(option) && option.ArgumentGroupName == section)
                        {
                        ...
                        while (parent is not null)
                        {
                            if ((parentCommand = parent.Symbol as Command) is not null)
                            {
                                foreach (var option in parentCommand.Options)
                                {
                                    ...
                                    if (option.IsGlobal && !option.IsHidden && uniqueOptions.Add(option) && option.ArgumentGroupName == section)
                                    {
                                        options.Add(ctx.HelpBuilder.GetTwoColumnRow(option, ctx));
                                    }
                                }
                    ...
                    sizeOptions.AddRange(options);
                    groupedOptions.Add((section, options));
                }
                foreach (var (section, options) in groupedOptions)
                {
                    if (options.Count <= 0)
                    {
                        ctx.WasSectionSkipped = true;
                    ...
                    ctx.HelpBuilder.WriteHeading(section ?? ctx.HelpBuilder.LocalizationResources.HelpOptionsTitle(), null, ctx.Output);
                    ctx.HelpBuilder.WriteColumns(options, ctx, sizeOptions);
