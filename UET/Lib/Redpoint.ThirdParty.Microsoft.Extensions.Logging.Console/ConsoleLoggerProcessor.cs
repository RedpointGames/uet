    ...
    internal class ConsoleLoggerProcessor : IDisposable
    {
        ...
        public int MaxQueueLength
        {
            ...
            set
            {
                if (value <= 0)
                {
                    throw new ArgumentOutOfRangeException(string.Format("{0} must be larger than zero.", nameof(value)));
                }
        ...
        public ConsoleLoggerQueueFullMode FullMode
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(string.Format("{0} is not a supported queue mode value.", nameof(value)));
                }
                ...
                if (!_isAddingCompleted)
                {
                    ...
                    if (_messagesDropped > 0)
                    {
                        _messageQueue.Enqueue(new LogMessageEntry(
                            message: string.Format("{0} message(s) dropped because of queue size limit. Increase the queue size or decrease logging verbosity to avoid this. You may change `ConsoleLoggerQueueFullMode` to stop dropping messages." + Environment.NewLine, _messagesDropped),
                            logAsError: true
                        ));
