namespace Redpoint.CloudFramework.Locking
{
    ...
    public interface IGlobalLockService
    {
        ...
        Task<ILockHandle> Acquire(string @namespace, UntypedKey objectToLock);

        /// <summary>
        /// Acquires a lock in the given namespace using <see cref="Acquire(string, UntypedKey)"/>, and then calls
        ...
        Task AcquireAndUse(string @namespace, UntypedKey objectToLock, Func<Task> block);
        ...
        Task<T> AcquireAndUse<T>(string @namespace, UntypedKey objectToLock, Func<Task<T>> block);
    }
}
