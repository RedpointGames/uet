namespace Redpoint.Uet.BuildPipeline.BuildGraph
{
    ...
    internal class DefaultBuildGraphExecutor : IBuildGraphExecutor
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                var allowUnifiedBuildGraph = true;
                ...
                if (System.Environment.GetEnvironmentVariable("UET_DISALLOW_UNIFIED_BUILDGRAPH") == "1")
                {
                    allowUnifiedBuildGraph = false;
                    _logger.LogInformation("This build will run with the non-unified BuildGraph, as the environment variable UET_DISALLOW_UNIFIED_BUILDGRAPH=1 is set.");
                }
                else if (allowUnifiedBuildGraph)
                {
                ...
                buildGraphArguments["UseUnifiedBuild"] = allowUnifiedBuildGraph ? "true" : "false";
